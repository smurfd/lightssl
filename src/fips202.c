...
void KeccakF1600_StatePermute(u64 state[25]) {
  ...
  u64 Aba, Abe, Abi, Abo, Abu, Aga, Age, Agi, Ago, Agu, Aka, Ake, Aki, Ako, Aku;
  u64 Ama, Ame, Ami, Amo, Amu, Asa, Ase, Asi, Aso, Asu, BCa, BCe, BCi, BCo, BCu;
  u64 Da, De, Di, Do, Du, Eba, Ebe, Ebi, Ebo, Ebu, Ega, Ege, Egi, Ego, Egu;
  u64 Eka, Eke, Eki, Eko, Eku, Ema, Eme, Emi, Emo, Emu, Esa, Ese, Esi, Eso, Esu;
...
static void keccak_absorb(u64 s[25], unsigned int r, const uint8_t *m, size_t mlen, uint8_t p) {
  size_t i;
  uint8_t t[200] = {0};
...
static void keccak_squeezeblocks(uint8_t *out, size_t nblocks, u64 s[25], unsigned int r) {
  unsigned int i;
  while(nblocks > 0) {
    KeccakF1600_StatePermute(s);
    for(i=0;i<r/8;i++)
