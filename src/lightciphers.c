...
const uint8_t SBOX[16][16] =  {{0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
  ...
  0x6d, 0x8d, 0xd5, 0x4e, 0xa9, 0x6c, 0x56, 0xf4, 0xea, 0x65, 0x7a, 0xae, 0x08},
 {0xba, 0x78, 0x25, 0x2e, 0x1c, 0xa6, 0xb4, 0xc6, 0xe8, 0xdd, 0x74, 0x1f, 0x4b,
  ...
  0xbb, 0x16}};
const uint8_t SBOXINV[16][16] =     {{0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5,
  0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb},     {0x7c, 0xe3, 0x39,
  0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb},
 {0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42,
  ...
  0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4},
...
const uint8_t GF[15][256] = {{0x0}, {0x0},
 ...
 {0x00, 0x09, 0x12, 0x1b, 0x24, 0x2d, 0x36, 0x3f, 0x48, 0x41, 0x5a, 0x53, 0x6c,
  ...
  0x0e, 0x79, 0x70, 0x6b, 0x62, 0x5d, 0x54, 0x4f, 0x46}, {0x0},
  // mul 11
 {0x00, 0x0b, 0x16, 0x1d, 0x2c, 0x27, 0x3a, 0x31, 0x58, 0x53, 0x4e, 0x45, 0x74,
  ...
  0xfb, 0x92, 0x99, 0x84, 0x8f, 0xbe, 0xb5, 0xa8, 0xa3}, {0x0},
  // mul 13
...
const uint8_t MIX[4][4]={{2, 3, 1, 1}, {1, 2, 3, 1}, {1, 1, 2, 3}, {3, 1, 1, 2}};
const uint8_t MIXINV[4][4]={{14,11,13,9},{9,14,11,13},{13,9,14,11},{11,13,9,14}};

//
// Copy a state array to another
static void copy_state(uint8_t s[4][NB], uint8_t in[4][NB]) {
  for (int j = 0; j < 4; ++j) for (int i = 0; i < NB; ++i) {s[j][i] = in[j][i];}
}
...
//
//
static void lpshiftrow(uint8_t state[4][NB], uint32_t i, uint32_t n) {
  uint8_t tmp[NB];
...
//
// 5.3.x
static void lpinvshiftrows(uint8_t state[4][NB]) { // See Sec. 5.3.1
  for (int i = 1; i <= 3; i++) {lpshiftrow(state, i, NB - i);}
}
...
//
//
static void lpshiftrows(uint8_t state[4][NB]) { // See Sec. 5.1.2
  for (int i = 1; i <= 3; i++) {lpshiftrow(state, i, i);}
}
...
//
//
static void lpsubword(uint8_t *wrd) {
  for (int i = 0; i < 4; i++) {wrd[i] = SBOX[wrd[i] / 16][wrd[i] % 16];}
}

//
//
static void lprotword(uint8_t *wrd) {
  uint8_t tmp = wrd[0];
...
//
//
static void lprcon(uint8_t *wrd, uint8_t a) {
  uint8_t c = 1;
...
//
//
static void lpkeyexpansion(uint8_t key[NK * 2], uint8_t w[NB * NR]) {
  uint8_t tmp[4], rc[4];
...
static void lpdecryptblock(uint8_t in[BBL], uint8_t out[BBL], uint8_t *rk) {
  ...
  for (uint32_t round = NR - 1; round >= 1; round--) {
    lpinvsubbytes(state);
    lpinvshiftrows(state);
    lpaddroundkey(state, rk + round * 4 * NB);
    lpinvmixcolumns(state);
  }
  lpinvsubbytes(state);
  lpinvshiftrows(state);
  lpaddroundkey(state, rk);
