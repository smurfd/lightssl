...
uint32_t get_tlv_len(const uint8_t *tlv, uint32_t tlvlen, uint32_t *tlvoff) {
  ...
  *tlvoff = 0;
  ...
  if (tlvlen < 2) return 0xFFFFFFFF;
  ++tlv; ++(*tlvoff);
  a = *tlv++; ++(*tlvoff);
  if (a < 128) return a + (*tlvoff); // <128 means character
  a &= 0x7F;
  tlvlen -= 2;
  if (a == 0 || a > 4 || a > tlvlen) return 0xFFFFFFFF;
  *tlvoff += a;
  b = 0;
  while ((a--) > 0) {b = (b << 8) | ((uint32_t)*tlv); ++tlv;};
  if (UINT32_MAX - (*tlvoff) < b) return 0xFFFFFFFF; // check to not return overflow
  return b + (*tlvoff);
}
...
uint32_t get_data_len(const uint8_t *data, uint32_t datalen) {
  ...
  if (datalen < 2) return 0xFFFFFFFF;
  ++data;
  a = *data++;
  if (a < 128) return a; // <128 means character
  a &= 0x7F;
  datalen -= 2;
  if (a == 0 || a > 4 || a > datalen) return 0xFFFFFFFF;
  b = 0;
...
uint32_t get_len_enc_len(uint32_t datalen) {
  ...
  if (datalen < 128) {len = 1;}
  else {
    len = 1;
    uint32_t len1 = datalen;
    do {
      len1 = len >> 8;
      ++len;
    } while(len1 > 0);
...
uint32_t get_der_enc_len(asn_tree *asn) {return (1 + asn->len + get_len_enc_len(asn->len));}
...
int32_t der_objcnt(const uint8_t *der, uint32_t derlen) {
  uint32_t deroff, derenc_len = get_tlv_len(der, derlen, &deroff), derdat_len = derenc_len - deroff;
  int32_t objcnt = 1;
  ...
  if ((*der & 0x20) != 0) {
    ...
    while (children_len < derdat_len) {
      ...
      uint32_t child_len = get_tlv_len(child, child_maxlen, &childoff);
...
int32_t der_dec(const uint8_t *der, uint32_t derlen, asn_tree *out, asn_tree *outobj, unsigned int outobjcnt) {
  ...
  if ((*der & 0x20) != 0) {
    ...
    while (children_len < derdat_len) {
      const uint8_t *child = (der + deroff);
      uint32_t child_datoff, child_maxlen = (derenc_len - deroff), child_len = get_tlv_len(child, child_maxlen, &child_datoff);
      int32_t child_obj = der_objcnt(child, child_len);
...
int dump_and_parse(uint8_t *cmsd, uint32_t fs) {
  ...
  asn_tree cms;
  if (der_dec(cmsd, fs, &cms, asnobj, objcnt) < 0) {printf("cant parse data\n"); free(cmsd); return 1;}
  printf("----- dump begin ----\n");
  printasn(&cms, 0);
  printf("----- dump end ----\n");
  printf("----- parse begin ----\n");
  if (cms.type != ASN1_TYPE_SEQUENCE) {printf("outr type != SEQUENCE, %x\n", cms.type); return 1;}
  ...
  printf("Content type: ");
  if (memcmp(ct->data, (uint8_t[]){0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x07, 0x06, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x07, 0x06}, ct->len) != 0) {
    printf("no contenttype of type encrypteddata avail\n"); return 1;
  }
  printf("encryptedData\n");
  asn_tree *encd = ct->next->child;
  if (encd == NULL || encd->type != ASN1_TYPE_SEQUENCE) {printf("no encrypted data avail\n"); return 1;}
  ...
  if (enccict->len != 9 || memcmp(enccict->data, (uint8_t[]){0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x07, 0x01}, enccict->len) != 0) {
  ...
  if (ctencalg == NULL) {printf("no content encryption algo avail\n"); return 1;}
  if (ctencalg->type == ASN1_TYPE_SEQUENCE) {
    ...
    if (memcmp(encalgi->data, (uint8_t[]){0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x01, 0x02}, encalgi->len) == 0) {
      printf("content encrypt algo: AES-128-CBC\n");
      asn_tree *aesiv = encalgi->next;
      if (aesiv == NULL || aesiv->type != ASN1_TYPE_OCTET_STRING) {printf("no aes iv avail\n"); return 1;}
      printf("AES IV:\n");
      printhex(aesiv->data, aesiv->len);
    } else {printf("unknown encryption algo\n");}
    asn_tree *encct = ctencalg->next;
    if (encct == NULL || encct->type != 0x80) {printf("no encr content avail\n"); return 1;}
