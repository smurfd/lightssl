...
// Error handling
static int hmac_error(hmacctx *c) {
...
static int sha_error(shactx *c, cu8 *msg_arr, unsigned int length, int b) {
  if (!c) return sha_null;
...
void hash_new(cc *in, char* s) {
  cc hexdigits[] = "0123456789ABCDEF";
  u08* inn = malloc(sizeof(u08) * strlen(in));
  u08 msg_dig[sha_hsh_sz];
  shactx sha;
  int j = 0;
...
//
//
int sha_input(shactx *c, cu8 *msg_arr, unsigned int length) {
  u64 tmp;
...
int sha_final(shactx *c, u08 msg_bit, unsigned int length) {
  ...
  sha_error(c, (const u08 *)0, length, 1);
